...
for hc_dataset_name in hc_datasets:
    for i in range(5):
        ...
        if not os.path.exists(f"./hc_conversion_results_T16/{hc_dataset_name}"):  
            os.makedirs(f"./hc_conversion_results_T16/{hc_dataset_name}")
        score_file_name = f'TD3_HalfCheetah-v3_{i}'
        np.save(f"./hc_conversion_results_T16/{hc_dataset_name}/{score_file_name}", hc_score)
